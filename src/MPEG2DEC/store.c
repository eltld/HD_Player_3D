...
//===========================================================================
extern unsigned char Dec_Buffer[720*2880*3];
extern unsigned char Dec_Buffer_[1920*1024*3];
extern BOOL volatile Decoding;
...
//#define OBFRSIZE 777600+18 //改变解码数据存储空间的大小
//#define OBFRSIZE 720*2880*3+18 //改变解码数据存储空间的大小
#define OBFRSIZE 1920*1024*3+18 //改变解码数据存储空间的大小
...
int tgaflag;
{
	...
	int crv, cbu, cgu, cgv;
	unsigned char *py, *pu, *pv;
	static unsigned char tga24[14] = {0,0,2,0,0,0,0, 0,0,0,0,0,24,32};//==========这里是 TGA 的头信息
	...
	strcat(outname,tgaflag ? ".tga" : ".ppm");//=============将扩展名添加到文件名末尾
	...
	if (tgaflag)//==============================将图像的 尺寸 信息 写到TGA文件头中
	{
		/* TGA header */
		for (i=0; i<12; i++)
			putbyte(tga24[i]);//=================================================================
	...
	for (i=0; i<height; i++)
	{
		...
		for (j=0; j<horizontal_size; j++)
		{
			...
			if (tgaflag)
			{
				putbyte(b); putbyte(g); putbyte(r);//============================================这里是写入 BGR 数据
			}
			else
			{
	...
	{
//		write(outfile,obfr,optr-obfr);//========================================
		//此处是把剩余在buffer中尚未存满而未写入文件的数据 写入文件 属于收尾工作
	}
...
static void putbyte(c)//=================================================很多次调用这个函数写数据
int c;
{
	*optr++ = c;

	if (optr == obfr+OBFRSIZE)//解码 buffer 里面的 RGB数据 存满一帧了
	{
//		write(outfile,obfr,OBFRSIZE);//=================================================这里有写文件操作

		memcpy(Dec_Buffer_,obfr+18,OBFRSIZE-18);//18 是 TGA 信息头的长度
		//这里 原来的写文件操作时经过多次循环才把一帧的图像数据写进文件的 因为 OBFRSIZE 太小了 所以需要多次用
		//现在改成 每次写文件都是存储一整帧的数据

		optr = obfr;
		Decoded_Num=Decoded_Num+1;
		Decoding = 0;//============************此处无法改变Decoding 的值!!!!!! 可以改变了 解决了该问题
	}
}
